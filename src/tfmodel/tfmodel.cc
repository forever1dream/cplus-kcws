...
bool TfModel::Eval(const std::vector<std::pair<std::string, tensorflow::Tensor> >& inputTensors,
                    const std::vector<std::string>& outputNames,
                    std::vector<tensorflow::Tensor>& outputTensors) 
{
    tensorflow::Status s = session_->Run(inputTensors, outputNames, {}, &outputTensors);
    if (!s.ok()) {
