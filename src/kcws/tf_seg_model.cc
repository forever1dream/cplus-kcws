 ...
 * Author:  cdsunxiaoxiong
 * Create Time: 2018-03-08 10:31:03
 * Description:
 *
 */
...
public:
	...
	void fakePredication(Eigen::TensorMap<Eigen::Tensor<float, 3, Eigen::RowMajor>,
			Eigen::Aligned>& predictions, int sentenceIdx) 
	{
...
bool load_vocab(const std::string& path, std::unordered_map<UnicodeCharT, int>* pVocab) 
{
...
bool TfSegModel::LoadModel(const std::string& modelPath,const std::string& vocabPath, 
		int maxSentenceLen, const std::string& userDictPath) 
{
		...
		std::vector<std::vector<SegTok>>* pTopResults) 
{
	...
	//std::cout << "Segment:" << sentences.size() << std::endl;
	// Create input tensor
...
bool TfSegModel::Segment(const std::string& sentence,
	...
	//std::cout << "unicodestr ustr:" << ustr.c_str() << std::endl;
	...
	for (size_t i = 0; i < ns; i++) {
		...
		if (pTags != nullptr && tagger_) {
			...
			if (!tagger_->Tag(todos, tags)) {
                std::cout << "pos tagger error" << std::endl;
                return false;
            }

            CHECK_EQ(tags[0].size(), todo.size()) << "pos tagger size not match";
			//if (tags[0].size(), todo.size()) {
             //   std::cout << "pos tagger size not match" << std::endl;
            //    return false;
            //}
			for (auto t : tags[0]) {
				pTags->push_back(t);
