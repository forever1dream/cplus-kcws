...
static void convert_string_to_5_chars(const std::string& word, WordInfo& info, 
...
static bool load_word_vocab(const std::string& path, std::unordered_map<std::string, WordInfo> * pVocab,
		const std::unordered_map<UnicodeCharT, int>& charVocob) 
{
  FILE *fp = fopen(path.c_str(), "r");
  if (fp == NULL) {
...
bool PosTagger::LoadModel(const std::string& modelPath, const std::string& wordVocabPath,
		const std::string& charVocabPath, const std::string& tagVocabPath, int maxSentenceLen) 
{
...
bool PosTagger::Tag(const std::vector<std::vector<std::string>>& sentences,
